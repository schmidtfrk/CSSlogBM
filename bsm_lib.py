...
#warning: carefully read the documentation whether a function counts
#succesfull or failed logical Bell measurements

"""
When considering arbitrary linear optical Bell measurements only the functions
'general_safe_bm', 'general_safe_bm_erasure' and
'general_safe_bm_erasure_advanced' need to be considered.
Other functions are historical remains, but some of those might be useful
performancewise when restricting to special cases.
For example if one considers only ZZ=1 measurements it could be useful
to use 'bsmprobability' because then do do not need to keep track of
the X-codewords.
"""
...
def optical_condition(codeword,conditions,informationplaces=0,addinfo=0):
    ''' Determines if the given codition is meet.
        ...
        Boolean
    '''
    #Function was modified in an ad-hoc manner in order to obtain compatibility
...
#scripts for calculating the logical transmission with loss but ideal measurements
